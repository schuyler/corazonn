...
class HeartbeatPredictor:
    ...
    def _process_observation(self, timestamp_s: float) -> None:
        ...
        Args:
            timestamp_s: Observation timestamp (seconds)
        """
        if self.ibi_estimate_ms is None or self.last_beat_time is None:
            # Shouldn't happen, but handle gracefully
            print(f"PPG {self.ppg_id}: Observation received but no IBI/beat baseline, ignoring")
            return

        # Calculate observed IBI (time since last beat)
        observed_ibi_ms = (timestamp_s - self.last_beat_time) * 1000.0

        # Validate observed IBI
